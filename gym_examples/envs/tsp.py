...
class TSPEnvironment(gym.Env):
    def __init__(self, num_cities):
        ...
        # Gym spaces
        self.action_space = spaces.Discrete(num_cities)
        self.observation_space = spaces.Discrete(num_cities)  # Changed observation space
    ...
    def reset(self):
        self.current_city = 0
        self.visited_cities = set([0])
        return self.current_city  # Return index of current city
    ...
    def step(self, action):
        ...
        next_state = self.current_city  # Next state is the index of the next city

        self.ax.clear()
        self.ax.scatter(self.city_coordinates[:, 0], self.city_coordinates[:, 1], color='blue', label='Cities')
        self.ax.scatter(*self.city_coordinates[self.current_city], color='green' if not done else 'red', marker='*' if not done else 'x', s=200, label='Next City')
        if not done:
            self.ax.scatter(*self.city_coordinates[[list(self.visited_cities)[0]]].T, color='orange', marker='o', s=200, label='Start City')
        self.ax.legend()
        plt.pause(0.001)  # Short pause for visualization
...
# Example usage:
env = TSPEnvironment(num_cities=10)
